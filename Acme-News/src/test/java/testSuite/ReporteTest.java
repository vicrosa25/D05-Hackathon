	...
	private NoticiaService noticiaService;
		...
		protected void templateCreateReporte(final Class<?> expected,final String username,final String noticiaTitulo) {
			...
			try {
				...
				Integer antes= reporteService.getReporteRepository().findAll().size();
				Noticia noticia=noticiaService.getNoticiaRepository().findOneByName(noticiaTitulo);
				Reporte reporte= reporteService.create(noticia);
				reporte.setTexto("Prueba testCreateReporteAuthenticateAsUsuario");
				Reporte reporte2=reporteService.save(reporte);	
				
				Integer despues=reporteService.getReporteRepository().findAll().size();
				Assert.isTrue(reporteService.getReporteRepository().findAll().contains(reporte2));
