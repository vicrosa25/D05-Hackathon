...
public interface AdminRepository extends JpaRepository<Administrador, Integer> {
	...
	@Query("select min(p.informacion.size),max(p.informacion.size),avg(p.informacion.size) from Periodista p")
	Object[] getQueryH1();
	...
	@Query("select p.nombre from Periodista p join p.informacion i WHERE i.class IN Noticia ORDER BY i.numeroVisitas DESC")
